...
public class NSRCG3_SR3_Loader {
	...
	public void ExportFile (String filename, PlayerCharacter pc) throws IOException {
		list = new ArrayList<String[]>();
		
		put ("Priorities.Attributes", pc.getString (PlayerCharacter.STAT_ATTR_POINTS));
		put ("Priorities.Skills", pc.getString (PlayerCharacter.STAT_SKILL_POINTS));

		put ("Race.RaceName", pc.getString (PlayerCharacter.STR_RACE));

		// [General]
		put ("General.CharName", pc.getString (PlayerCharacter.STR_CHARNAME));
		put ("General.RealName", pc.getString (PlayerCharacter.STR_REALNAME));
		put ("General.Creator", pc.getString (PlayerCharacter.STR_PLAYERNAME));
		put ("General.StreetName1", pc.getString (PlayerCharacter.STR_STREETNAME1));
		...
		for (int i = 0; i < strKeys.length; i++) {
			put ("General." + strKeys[i], pc.getString(PlayerCharacter.STR_AGE + i));
		}
		...
		for (int i = 0; i < 6; i++) {
			...
			for (int j=0; j < 4; j++) {
				value += ((j == 0) ? "" : "|" ) + pc.getString (PlayerCharacter.STAT_BOD_BASE + 5 * i + j);
			}
			put ("Attributes." + attrKeys[i], value);
		}
		...
		for (int i = 0; i < poolKeys.length; i++) {
			put ("Pools." + poolKeys[i], pc.getString(PlayerCharacter.STAT_POOL_COMBAT + i));
		}
		
		// [Karma]
		put ("Karma.TotalKarma", pc.getString(PlayerCharacter.STAT_KARMA_TOTAL));
		put ("Karma.RemainKarma", pc.getString(PlayerCharacter.STAT_KARMA_FREE));
		put ("Karma.SpentKarma", pc.getString(PlayerCharacter.STAT_KARMA_SPENT));
	...
	protected void writeFile (String filename) throws IOException {
		...
		try {
			...
			FileWriter tgt = new FileWriter (tgtfile);

			for (int i=0; i<list.size(); i++) {
				//String[] parts = line.split ("=", 2);
				String[] elems = list.get(i)[0].split ("\\.");
				if (!section.equalsIgnoreCase(elems[0])) {
					if (i > 0)
						tgt.write("\n");
					tgt.write("[" + elems[0] + "]\n");
					section = elems[0];
				}
				tgt.write (elems[1] + "=" + list.get(i)[1] + "\n");
			}
	        tgt.close();
	    } catch (IOException e) {
	    	//blocker.setBlocked(false);
	    	System.err.println("Can't save file: " + filename);
	    	throw e;
	    }	
	}
...
//	void saveStat (String key, PlayerCharacter pc, int index) {
//		put (key, pc.getString (index));		
//	}
}
